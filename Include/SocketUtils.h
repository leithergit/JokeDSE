...
// 		LeaveCriticalSection(&m_csRecvList);
	...
	int CreateAsyncSocket(long lNetworkEvents = FD_READ | FD_WRITE | FD_CONNECT | FD_CLOSE)
	{
		DWORD dwErrorCode = 0;
	...
	int Connect(const TCHAR *szServer,int nPort,int nTimeout = 5000,int nLocalPort = -1)
	{	
		DWORD dwErrorCode = 0;
		__try
		{
			...
			//int nNetTimeout = 500;//500毫秒
			//发送时限
			if (setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (char *)&nTimeout, sizeof(int)) == SOCKET_ERROR)
			{
				dwErrorCode = GetLastError();
				__leave;
			}
			...
			//接收时限
			if (setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (char *)&nTimeout, sizeof(int)) == SOCKET_ERROR)
			{
				dwErrorCode = GetLastError();
				__leave;
			}
			...
			unsigned long nNonBlock = 1;
			if( ioctlsocket(s, FIONBIO, &nNonBlock) != NO_ERROR)
			{
				dwErrorCode = GetLastError();
				__leave;
			}
			...
			int nSendBuf = 16384;
			if (setsockopt(s, SOL_SOCKET, SO_SNDBUF, (const char*)&nSendBuf, sizeof(int)) == SOCKET_ERROR)
			{
				dwErrorCode = GetLastError();
				__leave;
			}
		...
		__finally
		{
			if (dwErrorCode != 0 && s != INVALID_SOCKET)
			{
				DisConnect(s);
			}
		}
...
// 			}
	...
	bool EnableTCPDelay(BOOL nNoDelay = TRUE)
	{
		if (s == INVALID_SOCKET)
			return false;
		else
		{
			return (setsockopt(s, IPPROTO_TCP, TCP_NODELAY, (const char*)&nNoDelay, sizeof(int)) != SOCKET_ERROR);
		}
	}
...
//#endif
	...
	{		
		...
		CSockClient *pClient = pThreadInfo->pClientArray[dwIndex];
		::EnterCriticalSection(&pThreadInfo->csThread);		
		memmove(&pThreadInfo->hEventArray[dwIndex],&pThreadInfo->hEventArray[dwIndex + 1],(pThreadInfo->dwEventCount - dwIndex - 1)*sizeof(WSAEVENT));		
		memmove(&pThreadInfo->pClientArray[dwIndex],&pThreadInfo->pClientArray[dwIndex + 1],(pThreadInfo->dwEventCount - dwIndex - 1)*sizeof(CSockClient*));
		pThreadInfo->dwEventCount --;
