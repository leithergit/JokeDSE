...
public:
	...
	static UINT __stdcall ListenThread(void *p)
	{
		...
		while(pThis->m_bListenThreadRun)
		{
			...
			{
				// 客户端队列已满，需要排除等待
				EnterCriticalSection(&pThis->csClientCount);
				if (pThis->nClientCount < (__MaxThread * 64))
				{
					LeaveCriticalSection(&pThis->csClientCount);
					break;
				}
				else
				{ 
					LeaveCriticalSection(&pThis->csClientCount);
					TraceMsgA("%s Clients:%d,Client Queue is fulled.\r\n",__FUNCTION__, pThis->nClientCount);
					Sleep(50);
				}
			} while (pThis->m_bListenThreadRun);
